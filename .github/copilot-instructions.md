...
- List every source whose guidance, capability, or tools you applied to produce this reply.
- Use the file's `name` (from front-matter) when present; otherwise use the filename without extension. For MCP servers, use the server's key from `.vscode/mcp.json`.
- Categorise each entry with a prefix so the learner sees which mechanism fired:
  ...
  - `mcp:` for any MCP server in `.vscode/mcp.json` whose tools you called in this reply. Use `mcp:<server>` if you only connected/listed tools, or `mcp:<server>/<tool>` if you actually invoked a specific tool (preferred — it tells the learner which capability fired).

Example with an MCP tool call:

```
> 🧩 **Active context:** `repo:copilot-instructions`, `instructions:routes`, `mcp:github/get_pull_request`
```

Example with a skill plus an MCP fetch:

```
> 🧩 **Active context:** `repo:copilot-instructions`, `skill:vitest-test-generator`, `mcp:fetch/fetch`
```
...
2. Every source listed corresponds to a real file in `.github/` or a real server in `.vscode/mcp.json`.
...
5. If you called an MCP tool in this reply (e.g. fetched a URL via `fetch`, queried a PR via `github`), the matching `mcp:<server>/<tool>` entry is included. Conversely, do not list an MCP server you did not actually call — merely having it configured in `.vscode/mcp.json` is not enough.
